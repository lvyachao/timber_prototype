...
*/
		...
		else
		{
			...
			console.log($(basenode).children().length);
			for(var j=0;j<=$(basenode).children().length;j++)
					{
						console.log($(basenode).children()[j]);
...
function locatesameattributes(number)
{
	$(".forourtuple11").each(function()
	{
		...
		{
			var temppair=attributearray[number].partcrawlmarkarray[i].split(";;;;");
			var key=temppair[0].split(",")[0];
			var value=temppair[0].split(",")[1];
			var order=temppair[1].split(",")[1];
		//	console.log(order);
...
*/
						...
						node=$(node).children("#"+key[1])[order];
				...
				else if(key=="class")
				{
					...
					console.log(value1);
					node=$(node).children("."+value1)[order];
				}
				else if(key=="tag")
				{
					node=$(node).children(""+value)[order];
				}
				else
				{
